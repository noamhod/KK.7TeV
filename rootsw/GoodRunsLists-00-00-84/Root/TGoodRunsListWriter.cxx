...
Root::TGoodRunsListWriter::WriteXMLFile()
{
  ...
  if (m_dataCardName.IsNull()) {
    m_logger << kWARNING << "Output filename not set. Return false." << GEndl;
    return kFALSE;
  }
  ...
  //m_logger << kINFO << "To determine luminosity, type : iLumiCalc.exe -x " << m_dataCardName << GEndl;
...
Root::TGoodRunsListWriter::WriteNamedLumiRange(xmlTextWriterPtr writer)
{
  ...
  } else { m_logger << kWARNING << "<Version> written to xml file <" << m_dataCardName << "> is empty." << GEndl; }
  /* Write Metadata */
  ...
  } else { m_logger << kWARNING << "<Metadata> written to xml file <" << m_dataCardName << "> is empty." << GEndl; } 
  /* loop over goodruns */
