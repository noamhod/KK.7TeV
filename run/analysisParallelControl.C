...
#include "analysisParallelControl.h"


analysisParallelControl::analysisParallelControl()
{
...
void analysisParallelControl::setRunControl(string localRunControlFile)
{
	string sRun;
	string sRec;
	string sMCorData;
...
void analysisParallelControl::initialize()
{
	startTimer();
	
	string localRunControlFile = "localRunControl_noSkim.txt";
	...
	chainInit(vStr2find);
	makeChain(true, str_list, str_dir, 0);
...
void analysisParallelControl::finalize()
{
...
void analysisParallelControl::book()
{
...
void analysisParallelControl::draw()
{
...
void analysisParallelControl::fits()
{
	cout << "### in fits ####" << endl;
...
void analysisParallelControl::analyze()
{
	m_analysis->execute();
}

void analysisParallelControl::loop(Long64_t startEvent, Long64_t stopAfterNevents)
{
...
void analysisParallelControl::loop(int runNumber)
{
	if (m_WZphysD3PD->fChain == 0)  return;
...
void analysisParallelControl::loop(string sPeriodStart, string sPeriodEnd, Long64_t l64t_initialGuess)
{
	if (m_WZphysD3PD->fChain == 0)  return;
	...
	{
		cout << "ERROR in analysisParallelControl::loop(string sPeriodName): didn't find period with name = " << lastRunInRange << " or " << firstRunInRange << endl;
...
void analysisParallelControl::GetEntryMinimal(Long64_t entry)
{
