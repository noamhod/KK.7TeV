...
analysisControl::analysisControl()
{
	...
	makeChain(true, "/srv01/tau/hod/z0analysis-1.1.0/conf/dataset.list", "/data/hod/D3PDs/group10.phys-sm.data10_7TeV_physics_MuonswBeam_WZphys_D3PD/");

	m_phys = new physics( m_chain );


	m_treefile = new TFile("/srv01/tau/hod/z0analysis-1.1.0/data/analysisTrees.root", "RECREATE");
        m_treefile->cd();

	m_histfile = new TFile("/srv01/tau/hod/z0analysis-1.1.0/data/analysisControl.root", "RECREATE");
        m_histfile->cd();
	...
	m_GRL = new GRLinterface();
        m_GRL->glrinitialize("/srv01/tau/hod/z0analysis-1.1.0/conf/Z_GRL_152844-159224.xml");
	...
	m_analysis->readCutFlow("/srv01/tau/hod/z0analysis-1.1.0/conf/cutFlow.cuts");
