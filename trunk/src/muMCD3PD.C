...
muMCD3PD::muMCD3PD(mcPhysics* mcPhys, TFile* treeFile, TTree* tree)
{
	m_mcPhys = mcPhys;
...
TTree* muMCD3PD::getTree()
{
	return m_tree;
}

void muMCD3PD::setVectorPtrsMC()
{
...
void muMCD3PD::resetVectorPtrsMC()
{
...
{	
	...
	// staco truth
	int nStacoTruth = (int)m_mcPhys->mu_staco_truth_E->size();
	for(int i=0 ; i<nStacoTruth ; i++)
	{
		mu_staco_truth_E->push_back( m_mcPhys->mu_staco_truth_E->at(i) );
		mu_staco_truth_pt->push_back( m_mcPhys->mu_staco_truth_pt->at(i) );
		mu_staco_truth_eta->push_back( m_mcPhys->mu_staco_truth_eta->at(i) );
		mu_staco_truth_phi->push_back( m_mcPhys->mu_staco_truth_phi->at(i) );
		mu_staco_truth_type->push_back( m_mcPhys->mu_staco_truth_type->at(i) );
		mu_staco_truth_mothertype->push_back( m_mcPhys->mu_staco_truth_mothertype->at(i) );
		mu_staco_truth_barcode->push_back( m_mcPhys->mu_staco_truth_barcode->at(i) );
		mu_staco_truth_motherbarcode->push_back( m_mcPhys->mu_staco_truth_motherbarcode->at(i) );
		mu_staco_truth_status->push_back( m_mcPhys->mu_staco_truth_status->at(i) );
		mu_staco_truth_dr->push_back( m_mcPhys->mu_staco_truth_dr->at(i) );
		mu_staco_truth_matched->push_back( m_mcPhys->mu_staco_truth_matched->at(i) );
	}
	
	// muid truth
	int nMuidTruth = (int)m_mcPhys->mu_muid_truth_E->size();
	for(int i=0 ; i<nMuidTruth ; i++)
	{
		mu_muid_truth_E->push_back( m_mcPhys->mu_muid_truth_E->at(i) );
		mu_muid_truth_pt->push_back( m_mcPhys->mu_muid_truth_pt->at(i) );
		mu_muid_truth_eta->push_back( m_mcPhys->mu_muid_truth_eta->at(i) );
		mu_muid_truth_phi->push_back( m_mcPhys->mu_muid_truth_phi->at(i) );
		mu_muid_truth_type->push_back( m_mcPhys->mu_muid_truth_type->at(i) );
		mu_muid_truth_mothertype->push_back( m_mcPhys->mu_muid_truth_mothertype->at(i) );
		mu_muid_truth_barcode->push_back( m_mcPhys->mu_muid_truth_barcode->at(i) );
		mu_muid_truth_motherbarcode->push_back( m_mcPhys->mu_muid_truth_motherbarcode->at(i) );
		mu_muid_truth_status->push_back( m_mcPhys->mu_muid_truth_status->at(i) );
		mu_muid_truth_dr->push_back( m_mcPhys->mu_muid_truth_dr->at(i) );
		mu_muid_truth_matched->push_back( m_mcPhys->mu_muid_truth_matched->at(i) );
	}
	
	
	// muonTruth
	muonTruth_n = (int)m_mcPhys->muonTruth_n;
	int nMuonTruth = (int)m_mcPhys->muonTruth_pt->size();
	for(int i=0 ; i<nMuonTruth ; i++)
	{	
		muonTruth_pt->push_back( m_mcPhys->muonTruth_pt->at(i) );
		muonTruth_m->push_back( m_mcPhys->muonTruth_m->at(i) );
		muonTruth_eta->push_back( m_mcPhys->muonTruth_eta->at(i) );
		muonTruth_phi->push_back( m_mcPhys->muonTruth_phi->at(i) );
		muonTruth_charge->push_back( m_mcPhys->muonTruth_charge->at(i) );
		muonTruth_PDGID->push_back( m_mcPhys->muonTruth_PDGID->at(i) );
		muonTruth_barcode->push_back( m_mcPhys->muonTruth_barcode->at(i) );
		muonTruth_type->push_back( m_mcPhys->muonTruth_type->at(i) );
		muonTruth_origin->push_back( m_mcPhys->muonTruth_origin->at(i) );
	}
	
	// MC event
	mcevt_n = (int)m_mcPhys->mcevt_n;
	int nMCevt = (int)m_mcPhys->mcevt_signal_process_id->size();
	for(int i=0 ; i<nMCevt ; i++)
	{
		mcevt_signal_process_id->push_back( m_mcPhys->mcevt_signal_process_id->at(i) );
		mcevt_event_number->push_back( m_mcPhys->mcevt_event_number->at(i) );
		mcevt_event_scale->push_back( m_mcPhys->mcevt_event_scale->at(i) );
		mcevt_alphaQCD->push_back( m_mcPhys->mcevt_alphaQCD->at(i) );
		mcevt_alphaQED->push_back( m_mcPhys->mcevt_alphaQED->at(i) );
		mcevt_pdf_id1->push_back( m_mcPhys->mcevt_pdf_id1->at(i) );
		mcevt_pdf_id2->push_back( m_mcPhys->mcevt_pdf_id2->at(i) );
		mcevt_pdf_x1->push_back( m_mcPhys->mcevt_pdf_x1->at(i) );
		mcevt_pdf_x2->push_back( m_mcPhys->mcevt_pdf_x2->at(i) );
		mcevt_pdf_scale->push_back( m_mcPhys->mcevt_pdf_scale->at(i) );
		mcevt_pdf1->push_back( m_mcPhys->mcevt_pdf1->at(i) );
		mcevt_pdf2->push_back( m_mcPhys->mcevt_pdf2->at(i) );
		mcevt_weight->push_back( m_mcPhys->mcevt_weight->at(i) );
	}
}
